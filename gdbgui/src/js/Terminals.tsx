...
export class Terminals extends React.Component {
  ...
  terminal(ref: React.RefObject<any>, ids: string) {
    let className = " bg-black p-0 m-0 h-full align-baseline ";
    return (
      <div className={className} id={ids}>
        <div className="absolute h-full w-full align-baseline  " ref={ref}></div>
      </div>
    );
  }
  render() {
    let terminalsClass = "w-full h-full relative grid grid-cols-12 ";
    return (
      <div className={terminalsClass}>
        {this.terminal(this.userPtyRef, "termone")}
        {/* <GdbGuiTerminal /> */}
        {this.terminal(this.gdbguiPtyRef, "termtwo")}
        {this.terminal(this.programPtyRef, "termthree")}
      </div>
    );
  }
